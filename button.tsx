...
function Button({ className, variant = 'default', size = 'default', asChild, render, children, ...props }: ButtonProps) {
  const computedRender = render || (asChild && React.isValidElement(children) ? children : undefined)
  const computedChildren = asChild && React.isValidElement(children) ? undefined : children
  return (
    <ButtonPrimitive data-slot="button" className={cn(buttonVariants({ variant, size, className }))} render={computedRender} {...props}>
      {computedChildren}
    </ButtonPrimitive>
  )
}
